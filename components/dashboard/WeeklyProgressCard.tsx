    ...
    try {
      ...
      if (data.memories) {
        ...
        data.memories.forEach((m: any) => {
          ...
          // Only count memories from this week (last 7 days)
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="flex flex-col items-end gap-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-3xl font-bold text-white">{totalThisWeek}</span>
            </div>
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full">
                <div className="w-full relative" style={{ height: '100px' }}>
                  <div 
                    className={`absolute bottom-0 w-full rounded-t-xl transition-all shadow-lg ${
                      isToday ? 'bg-yellow-400' : count > 0 ? 'bg-purple-500' : 'bg-gray-700/50'
                    }`}
                    style={{ height: `${height > 0 ? Math.max(height, 10) : 0}%` }}
                  />
                </div>
